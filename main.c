...
int main(int argc, char* argv[]){ 
	int dimensions, check=0, dict=0, draw=0, count_slots=0, ind_dict=0;  
	char** crossw; 
	Slot* myslots;
	FILE* crossword_file=NULL;
	...
	int* needed_lengths = calloc(33 , sizeof(int)); 	//Πίνακας όπου στην θέση i έχει 1 αν το μήκος i 
	if(needed_lengths == NULL){                         //χρειάζεται για το σταυρόλεξο αλλιώς 0
    ...
    int max_slots = dimensions*dimensions/2 + 1;
	if((myslots = malloc(max_slots * sizeof(Slot))) == NULL){
        fprintf(stderr , "Error allocating memory\n");
        exit(EXIT_FAILURE);
    }
	int ind, first, col, length, row;
    ...
    if(check){ //check mode
		...
		int num_of_words[33], save_words[33];
		int**** bdict = malloc(33 * sizeof(int***));
		char*** words = malloc(33 * sizeof(char**));        //Βασικός πίνακας αποθήκευσης συμβολοσειρών με κριτήριο το μέγεθος τους
		char* str = malloc(33 * sizeof(char));
		if(bdict == NULL || words == NULL || str == NULL){  //bdict: Βοηθητικός πίνακας για την γρήγορη εύρεση του domain κάθε slot. 
		...
		while(fgets(str , 33 , dictionary_file) != NULL){ 
			str[strlen(str) - 1] = '\0';
			if(needed_lengths[strlen(str)]){
				num_of_words[strlen(str)]++;
			}
		}
		...
		while(fgets(str , 33 , dictionary_file) != NULL){ 
			...
			if(needed_lengths[length]){ 
				...
				for(int position = 0; position < length; position++){ //Αρχικοποίηση του bdict
					int ch = str[position] - 'a';
		...
		for(int i = 0; i < count_slots; i++){ 
			...
			for(int j = 0; j < num_of_words[myslots[i].length]; j++){
