...
//Εύρεση του επόμενου slot για συμπλήρωση με την μέθοδο MRV-heuristics
...
//Εύρεση των θέσεων που είναι conflicts ενός slot και αρχικοποίηση του πίνακα conflicts του
...
//Εύρεση των slot που επηρρεάζει κάθε slot
void find_neighbors(char** table, Slot* slots_table, int n_myslots){ 
	int row, col, is_horizontal, length, search_row, search_col, start, pos, letter; 
...
//Ελεγχος για το εάν μία λέξη είναι έγκυρη για το slot ή όχι
int slot_is_okey(Slot myslot, int word, Slot* slots_table, int n_myslots, int* lengths, int**** bdict, char*** my_dict){
	int length, j, count, pos_myslot, pos_neighbor, ch;                     
	fc_Listptr temp;    	//Για κάθε συμβατή λέξη του domain του slot ελέγχω αν τα domains των slot που επηρρεάζει είναι κενά			 
	length = myslot.length; //Εάν είναι κενά τότε ελέγχω μία άλλη συμβατή λέξη για το slot. Αν όχι, η λέξη είναι έγκυρη.
	for(int sl_ind = 0; sl_ind < n_myslots; sl_ind++){
		if(myslot.neighbors[sl_ind] == 1 && slots_table[sl_ind].filled == 0){     //Το slot με index: sl_ind στον πίνακα myslots επηρρεάζεται απο το τρέχων slot
			...
			for(j = 0; j < lengths[slots_table[sl_ind].length]; j++){ 
				...
				if(slots_table[sl_ind].current_domain[j] == 1){      //Για την ενημέρσωση του current_domain γίνεται λογική πράξη and.Αν είναι 1 η λέξη είναι συμβατή       
					count++;										 //Στον πίνακα bdict πάμε στις λέξεις που αντιστοιχούν στο μήκος
				}												     //slots_table[sl_ind].length και στην θέση pos_neighbor έχουν το γράμμα ch
			}
			slots_table[sl_ind].remaining_words = count;
			if(count == 0){ //Αν το domain του slot δεν έχει καμία συμβατή λέξη , τότε επαναφέρω όλα τα domains των slots που έχω αλάγξει μέχρι τώρα
				for(int slot_index = 0; slot_index <= sl_ind; slot_index++){
					if(myslot.neighbors[slot_index] == 1 && slots_table[slot_index].filled == 0){
						for(j = 0; j < lengths[slots_table[slot_index].length]; j++){
